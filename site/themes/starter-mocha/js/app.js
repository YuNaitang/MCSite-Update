/**
 * Starter_Mocha - Catppuccin Mocha 主题
 */
;(function () {
  'use strict'
  ...
  function applyThemeSettings(ts, cs = {}) {
    ...
    function setHeroColors(mode) {
      if (!hero) return
      const isDark = mode === 'dark'
      hero.style.setProperty('--hero-text', isDark ? '#cdd6f4' : '#cdd6f4')
      hero.style.setProperty('--hero-text-sub', isDark ? '#bac2de' : '#bac2de')
      hero.style.setProperty('--hero-text-muted', isDark ? '#6c7086' : '#6c7086')
      hero.style.setProperty('--hero-divider', isDark ? 'rgba(69,71,90,0.5)' : 'rgba(69,71,90,0.3)')
      hero.style.setProperty('--hero-addr-bg', isDark ? 'rgba(49,50,68,0.85)' : 'rgba(49,50,68,0.8)')
      hero.style.setProperty('--hero-addr-border', isDark ? 'rgba(69,71,90,0.4)' : 'rgba(69,71,90,0.3)')
      hero.style.setProperty('--hero-btn-bg', isDark ? '#cdd6f4' : '#cdd6f4')
      hero.style.setProperty('--hero-btn-color', isDark ? '#1e1e2e' : '#1e1e2e')
      hero.classList.toggle('hero--dark', isDark)
      hero.classList.toggle('hero--light', !isDark)
      const navbar = document.getElementById('navbar')
      if (navbar) navbar.classList.toggle('navbar--dark', isDark)
    }

    const heroImg = typeof ts.hero_image === 'string' ? ts.hero_image.trim() : ''
    const heroStyle = heroImg ? 'image' : (ts.hero_style || 'dark')

    document.querySelectorAll('.hero-overlay-layer').forEach(el => el.remove())

    if (heroStyle === 'dark') {
      setHeroColors('dark')
      if (heroBg) heroBg.style.background = '#1e1e2e'
    } else if (heroStyle === 'gradient') {
      setHeroColors('dark')
      if (heroBg) heroBg.style.background = 'linear-gradient(180deg, #181825 0%, #1e1e2e 100%)'
      ...
      setHeroColors(overlay === 'light' ? 'light' : 'dark')
    } else {
      setHeroColors('dark')
    }
    ...
    if (ts.motd_display === 'plain') {
      const motd = document.querySelector('.hero-motd')
      if (motd) { motd.style.background = 'transparent'; motd.style.color = 'var(--text-muted)'; motd.style.padding = '0' }
    }
    ...
    try {
      ...
      chart.setOption({
        ...
        tooltip: {
          ...
          extraCssText: 'backdrop-filter:blur(12px);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,0.3);',
        },
        ...
        xAxis: {
          ...
          axisLabel: { interval: 'auto', rotate: 0, color: '#6c7086' },
          axisLine: { lineStyle: { color: 'rgba(69,71,90,0.3)' } },
        },
        yAxis: {
          type: 'value',
          minInterval: 1,
          splitLine: { lineStyle: { color: 'rgba(69,71,90,0.15)' } },
        ...
        series: [
          {
            ...
            smooth: true,
            areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: 'rgba(205,214,244,0.12)' }, { offset: 1, color: 'transparent' }] } },
            lineStyle: { width: 2, color: '#cdd6f4' },
